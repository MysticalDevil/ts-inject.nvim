...
local debug_ok, debug_err = pcall(function()
  ...
  assert(text:find("diagnostics:", 1, true), "diagnostics section missing")
  assert(text:find("semantic tokens", 1, true), "expected 'semantic tokens' mention in diagnostics")
...
local health_ok, health_err = pcall(function()
  ...
  assert(text:find("semantic_token risk:", 1, true), "semantic_token risk section missing")
  vim.api.nvim_set_current_buf(original_buf)
end)
if health_ok then
